...
for temp_ratio = 1:length(ratio_bin)
    ...
    parfor temp_exp = 1:num_exp
        ...
        while length(data2_temp) < n2
            temp_data = normrnd(mu(2), sigma(2), 1, 100);
            temp_data = ceil(temp_data * range_half*2);
            temp_data(temp_data > range_half) = [];
            temp_data(temp_data < -range_half+1) = [];
...
delete(poolobj)
figure, 
semilogy(N_bin, delta_smooth_all(:,4))

save(['GG_012_',num2str(num_sample),'_',num2str(num_exp),'.mat'],'epsilon_bin','delta','delta_smooth_all','delta_save')